...
```bash
dxpcli wallet [create|inspect] [flags]
```
...
```bash
$ dxpcli wallet create --raw-entropy --root-only --words 15 --language english
```
...
```bash
$ dxpcli wallet inspect --raw-entropy --root-only --language english --password "Substrate" --mnemonic "abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon abandon about"
```

This command also leverages the BIP-0032 library for hierarchically derived wallets.

```bash
$ dxpcli wallet create --path "m/44'/0'/0'" --addresses 5
```
