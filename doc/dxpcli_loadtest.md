...
```bash
$ dxpcli wallet inspect  --mnemonic "code code code code code code code code code code code quality" --addresses 1
```
...
```bash
$ dxpcli loadtest --verbosity 700 --chain-id 1256 --concurrency 1 --requests 1000 --rate-limit 1 --mode t --rpc-url http://localhost:8888
```
...
```bash
$ dxpcli loadtest --verbosity 700 --chain-id 1256 --concurrency 1 --requests 50 --rate-limit 0.5  --mode f --function 164 --iterations 25078 --rpc-url http://private.validator-001.devnet02.pos-v3.polygon.private:8545
```
...
4. Run the loadtester to enure it deploys and runs successfully
   - `$ dxpcli loadtest --verbosity 700 --rpc-url http://127.0.0.1:8541`
...
- [dxpcli loadtest uniswapv3](dxpcli_loadtest_uniswapv3.md) - Run Uniswapv3-like load test against an Eth/EVm style JSON-RPC endpoint.
