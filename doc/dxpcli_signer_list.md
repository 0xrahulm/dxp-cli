...
```bash
dxpcli signer list --keystore /tmp/keystore
```

In the case of GCP KMS, the keyring will need to be provided and the keys can be listed with this command:

```bash
dxpcli signer list --kms GCP --gcp-project-id prj-polygonlabs-devtools-dev --gcp-keyring-id dxpcli-keyring
```
